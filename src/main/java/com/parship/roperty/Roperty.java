...
public interface Roperty {
    ...
    /**
     * Get a value for a given key from Roperty. Same as calling get(key, null, resolver);
     *
     * @param key      key to query
     * @param resolver resolver to determine domain values to use during resolution
     * @param <T>      type of the objects stored under the provided key
    ...
    /**
     * Get a value for a given key from Roperty. When no value is found in Roperty, the provided default is stored in Roperty. Same as
     * calling getOfDefine(key, defaultValue, resolver, null);
     *
     * @param key          key to query
     * @param defaultValue defaultValue is returned, when no value for the key is found
     * @param resolver     resolver to determine domain values to use during resolution
    ...
    /**
     * Get a value for a given key from Roperty. When no value is found in Roperty, the provided default is stored in Roperty along with the
     * provided description. Same as calling get(key, null, resolver);
     *
     * @param <T>          type of the objects stored under the provided key
     * @param key          key to query
     * @param defaultValue defaultValue is returned, when no value for the key is found
    ...
    /**
     * Get those KeyValues stored in this Roperty instance, with only those DomainSpecificValues, where the provided resolver domains match
